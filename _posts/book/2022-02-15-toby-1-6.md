...
- **빈(Bean)**{:.bg-blue .outline} : 스프링이 제어권을 가지고 직접 만들고 관계를 부여하는 오브젝트 **제어의 역전이 적용된 오브젝트**{:.pre-result .underline}
- **빈 팩토리**{:.bg-blue .outline} : 스프링에서 빈의 생성과 관계설정 같은 제어를 담당하는 IoC 오브젝트  
- **애플리케이션 컨텍스트**{:.bg-blue .outline} : 별도의 정보를 참고해서 빈의 생성, 관계설정 등의 제어 작업을 총괄하는 빈 팩토리
...
- DaoFactory를 설정정보로 사용하는 애플리케이션 컨텍스트
  ...
  - **@Configuration**{:.bg-ly .outline} : 스프링이 빈 팩토리를 위한 오브젝트 설정을 담당하는 클래스라고 인식  
  - **@Bean**{:.bg-ly .outline} : 오브젝트 생성을 담당하는 IoC용 메소드라는 표시  
  - {:.blank}
...
- **스프링 레지스트리**{:.bg-rose .outline} : 자바의 기본적인 싱글톤 패턴의 구현 방식은 여러 가지 단점이 있기 때문에, 스프링은 직접 싱글톤 형태의 오브젝트를 만들고 관리하는 기능을 제공한다.
...
- 기본적으로 싱글톤이 멀티스레드 환경에서 서비스 형태의 오브젝트로 사용되는 경우에는 상태정보를 내부에 갖고 있지 않은 **무상태**{:.bg-orange} 방식으로 만들어져야 한다.
